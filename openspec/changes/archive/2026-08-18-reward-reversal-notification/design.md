...
**`RewardReversalNotifier` does not install itself as the `UNUserNotificationCenter` delegate.** Surfaced during implementation, not anticipated up front: only one delegate can be installed on the shared center at a time, and `PendingRedemptionNotifier` already claims that role (to keep banners visible while the app is foregrounded). Its `willPresent` handler isn't scoped to the notifications it personally posts, so notifications from the new notifier get the same foreground presentation for free, as long as `PendingRedemptionNotifier` is also running — which `MainTabView` guarantees for every session regardless of role, since it isn't gated on `isParent` at construction, only inside its own `handle(_:)`. Rejected alternative: giving `RewardReversalNotifier` its own delegate conformance, which would silently steal the role and break parent-side banners depending on instantiation order — a strictly worse, order-dependent failure mode.
